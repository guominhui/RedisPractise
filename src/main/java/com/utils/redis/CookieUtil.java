package com.utils.redis;

import javax.servlet.http.Cookie;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import org.apache.commons.lang.StringUtils;

import lombok.extern.slf4j.Slf4j;

@Slf4j
public class CookieUtil {

	public final static String COOKIE_DOMAIN="guominhui.com";
	public final static String COOKIE_NAME="login_token";
	
	
	public static void writeLoginToken(HttpServletResponse response,String token){
		
		Cookie ck=new Cookie(COOKIE_NAME, token);
		ck.setDomain(COOKIE_DOMAIN);
		ck.setPath("/");//子域名下的文件路径
		ck.setHttpOnly(true);//防止脚本获取，或者发送第三方
		//单位是秒
		//如果这个maxage不设置的话，cookie就不会写入应硬盘，而是写在内存，只在当前页面有效。
	    ck.setMaxAge(60*60*24*365);//如果是-1，代表永久
	   // log.debug("write cookieName:{},cookiValue:{}",ck.getName(),ck.getValue());
	    response.addCookie(ck);
	}
	
	public static String readLoginToken(HttpServletRequest request){
		Cookie[] cks=request.getCookies();
		if(cks!=null){
			for(Cookie ck:cks){
				log.info("read cookieName:{},cookieValue:{}",ck.getName(),ck.getValue());
				if(StringUtils.equals(ck.getName(), COOKIE_NAME)){
					log.info("return cookieName:{},cookieValue:{}",ck.getName(),ck.getValue());
					return ck.getValue();
				}
			}
		}
		return null;	
	}
	//X: .happy.com             
	//a:A.happymmall.com        cookie:domain=A.happymmall.com;path="/"
	//b:B.happymmall.com        cookie:domain=B.happymmall.com;path="/"
	//c:A.happymall.com/test/cc   cookie:domain=A.happymmall.com;path="/test/cc"
	//d:A.happymall.com/test/dd   cookie:domain=A.happymmall.com;path="/test/dd"
	//e:A.happymall.com/test      cookie:domain=A.happymmall.com;path="/test"
	public static void delLoginToken(HttpServletRequest request,HttpServletResponse response){
		
		  Cookie[] cks=request.getCookies();
		  if(cks!=null){
			  for(Cookie ck:cks){
				  if(StringUtils.equals(ck.getName(), COOKIE_NAME)){
					  ck.setDomain(COOKIE_DOMAIN);
					  ck.setPath("/");
					  ck.setMaxAge(0);
					  log.info("return cookieName:{},cookieValue:{}",ck.getName(),ck.getValue());
				  response.addCookie(ck);
				  }
			  }
		  }
	}
	
}
