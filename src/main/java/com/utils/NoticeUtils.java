...
public class NoticeUtils {
	...
	public static void handleErrorWithOutNotice(HttpServletResponse response,String resultInfo){
		PrintWriter out;
		response.setContentType("text/html;charset=gb2312");
		try {
			out = response.getWriter();
			out.print("<script language=\"javascript\">alert('"+resultInfo+"');</script>");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public static void handleSuccessNoticeWithOut(HttpServletResponse response){
		PrintWriter out;
		response.setContentType("text/html;charset=gb2312");
		try {
			out = response.getWriter();
